...
export class Repository<T> { 
    ...
    async get (filter?: Partial<T>): Promise<T[]> {
        try {
            const response = await jsonServer.get<T[]>(`/${this.entityName}`, { params: filter })
    ...
    async update<T>(id: string, entity : Partial<Omit<T, 'id'>>): Promise<T> {
        try {
            const responseGet = await jsonServer.get<T[]>(`/${this.entityName}`, { params: { id } })
            const [ complete ] = responseGet.data
            const updated: T = {...complete, ...entity}
            const response = await jsonServer.put<T>(`/${this.entityName}/${id}`, updated)
            return response.data
        } catch (err) {
            console.error(err)
            throw err
